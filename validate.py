        ...
        for i, line in enumerate(lines):
            if lyref_tag in line:
                ...
                if mat and mat.start() != 0:
                    log.warning(r'\lyref before may be incorrect on line %d of %s', i+1, infile)
                mat = re_lyref_after.search(line)
                if mat and mat.end() != len(line)-1:
                    log.warning(r'\lyref after may be incorrect on line %d of %s', i+1, infile)

                mat = re_lyref_lyq.search(line)
                if not mat:
                    log.error(r'Incorrect \lyref structure on line %d of %s', i+1, infile)
                    errors += 1
                else:
                    ...
                    try:
                        ...
                        log.error(r'Incorrect \lyref label on line %d of %s', i+1, infile)
                        errors += 1
                    else:
                        if text and text not in special_lyrefs:
                            ...
                            if any(s not in texts[index] for s in snippets):
                                if '吾不试，故艺' in text:  # special case for quotation mark mismatch
                                    continue
                                log.error(r'Incorrect \lyref quoted text on line %d of %s', i+1, infile)
                                log.error(text)
...
def validate_lylinks(log):
    ...
    pat_link = re.compile(r'\\ly(?:char)?link\{(.+?)\}')
        ...
        for lineno, line in enumerate(io.open(infile, encoding=ENCODING)):
            for x in pat_link.finditer(line):
                ...
                if target not in anchors:
                    valid = False
                    log.error('Invalid target: "%s" on line %d of "%s"', target, lineno, infile)
    return valid
                ...
                while True:
                    ...
                    if not valid:
                        if not any(sp in line for sp in special_pinyin):
                            log.error('Incorrect pinyin on line %d, column %d of %s', i+1, index+1, infile)
                            return False
                        break
                    else:
                        index = right
